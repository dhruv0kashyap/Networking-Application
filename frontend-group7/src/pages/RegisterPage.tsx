...
const RegisterPage: React.FC = () => {
  ...
  const validate = (): FormErrors => {
    ...
    else if (!/^[A-Z][a-zA-Z]*$/.test(form.lastName))
      e.lastName = 'Last name must start with a capital letter and contain only English letters';
   ...
   if (!form.password) {
  e.password = 'Please provide a valid password';
} else if (form.password.length < 8 || form.password.length > 16) {
  e.password = 'Password must be 8–16 characters';
} else {
